...
type StackistanResumeProfileItem = StackistanResumeProfileCreate["activities"];
type StackistanResumeProfileItemRow =
  NonNullable<StackistanResumeProfileItem>["list"][number];
interface ResumeProfileActivitiesProps {
  items: StackistanResumeProfileItem;
  setItems: (items: StackistanResumeProfileItem) => void;
}

export function ResumeProfileActivities({
  items,
  setItems,
}: ResumeProfileActivitiesProps) {
    ...
    <div className="w-full h-full flex flex-col items-center justify-center">
      <div className="w-full  flex flex-col   gap-1 p-2 ">
        ...
        <p className="text-sm brightness-75">
          Highlight other activities like hacakthons or content creation
        </p>
      </div>
